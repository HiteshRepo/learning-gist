...
func TestAreTwoTreeSimilarInShape(t *testing.T) {
	...
	nums3 := []int{11, 21, 51, -1, 61, -1, -1, 31, 71, -1, 81, 111, -1, 121, -1, -1, 91, -1, -1, 41, -1, -1}
	tree3 := tree.ConstructGenericTree(nums3)

	isSimilar := tree.AreTwoTreeSimilarInShape(tree1, tree2)
	assert.True(t, isSimilar)

	isSimilar = tree.AreTwoTreeSimilarInShape(tree1, tree3)
